...
import './Component/matchGame/index.css'

import Navbar from './Component/navbar'

import DisplayImg from './Component/displayImg'
import ImageItem from './Component/imageCollection'

class App extends Component {
  state = {
    displayThumbnailId: 'FRUIT',
    isGameRunning: true,
    gameTimer: 60,
    score: 0,
    randomImage:
  ...
  onClickAnimal = () => {
    this.setState({displayThumbnailId: 'ANIMAL'})
  }
  onClickFruit = () => {
    this.setState({displayThumbnailId: 'FRUIT'})
  }
  onClickPlace = () => {
    this.setState({displayThumbnailId: 'PLACE'})
  }

  onClickThumbnail = (id, imageUrl) => {
    const {randomImage} = this.state
    if (imageUrl === randomImage) {
      const suffuledImageList = thumbnailList.sort(() => Math.random() - 0.5)
      this.setState(prevState => ({
        randomImage: suffuledImageList[0].imageUrl,
  ...
  render() {
    const {displayThumbnailId, gameTimer, randomImage, score, isGameRunning} =
      this.state
    const displaythumbnail = imagesList.filter(
      each => each.category === displayThumbnailId,
    )
    ...
    return (
      <div className="main-container">
        <ul>
          <Navbar gameTimer={gameTimer} gameScore={score} />
        </ul>
        {gameTimer > 0 ? (
          <div>
            <ul className="img-container">
              <DisplayImg image={randomImage} />
            </ul>
            <div>
              <li type="button" onClick={this.onClickFruit}>
                Fruits
              </li>
              <li type="button" onClick={this.onClickAnimal}>
                Animals
              </li>
              <li type="button" onClick={this.onClickPlace}>
                Places
              </li>
            </div>
            <ul className="imgItem-container">
              {displaythumbnail.map(each => (
                <ImageItem
                  eachImage={each}
                  key={each.id}
